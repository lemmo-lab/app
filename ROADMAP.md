...
- [ ] **تایپوگرافی ۴گانه اختصاصی:**
  - کانفیگ فونت‌های محلی با `next/font/local`:
    - فارسی: **Morabba** (عناوین) و **IRANSansX** (متن بدنه).
    - انگلیسی: **Oddval 600** (عناوین) و **Satoshi Variable** (متن بدنه).
- [ ] **موتور چندزبانه و جهت‌بندی پویا (i18n & RTL/LTR):**
  - استور کلاینت برای نگهداری زبان فعال کاربر (`fa` / `en`).
  - تنظیم خودکار تگ `<html dir="rtl" lang="fa">` یا `<html dir="ltr" lang="en">`.
